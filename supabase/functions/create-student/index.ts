...
// Mirrors create-mentor's response shape so both admin-account flows are consistent for the frontend.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (roleError || !callerRoles || (!callerRoles.includes("admin") && !callerRoles.includes("super_admin"))) {
      return createResponse(false, "FORBIDDEN", "Forbidden", null, ["Only admins can create students"], {}, 403, requestId);
    }
    ...
    const { fullName, email, password, phone, onboardingMethod } = parseResult.data;
    ...
    try {
      ...
      // Step B: Workspace Provisioning (Profile, Student Profile, User Settings, etc)
      ...
      // Student Profile (all columns are optional/defaulted — created empty, filled in later)
      const { error: studentProfileError } = await serviceClient
        .from("student_profiles")
        .upsert({ id: createdAuthUserId }, { onConflict: "id" });
      if (studentProfileError) throw new Error("Student Profile creation failed: " + studentProfileError.message);

      // Role mapping
      const { data: roleRecord, error: roleLookupError } = await serviceClient.from("roles").select("id").eq("code", "student").maybeSingle();
      if (roleLookupError) throw new Error("Student role lookup failed: " + roleLookupError.message);
      if (!roleRecord) throw new Error("Student role lookup failed: role code 'student' not found");
      ...
      // Audit Log (best-effort — a failure here should not fail the whole student creation)
      ...
      // return the temporary password so the admin can hand it to the student directly.
      // Never logged; only returned once, over HTTPS, to the authenticated admin caller.
      log(requestId, "info", "student_provisioned", { userId: createdAuthUserId, onboardingMethod });

      return createResponse(
        true,
        "OK",
        "Student account provisioned successfully",
      ...
      if (createdAuthUserId) {
        // Delete cascades to profiles/student_profiles/user_roles/user_settings/user_statistics/
        // via ON DELETE CASCADE FKs — but only if this call actually succeeds, so
        ...
        if (deleteResult.error) {
          ...
          return createResponse(
            false,
            "ROLLBACK_FAILED",
            "Student provisioning failed and automatic cleanup also failed. This account requires manual review.",
            null,
      ...
      return createResponse(
        false,
        "WORKSPACE_PROVISION_FAILED",
        "Failed to provision student account. System rolled back.",
        null,
        [sagaMessage],
        {},
        500,
