...
import { uploadFile, type StorageBucketKey } from "@/services/storage.service";
...
export class ProfileService {
  ...
  async uploadAvatar(userId: string, file: File, bucket: StorageBucketKey = "students"): Promise<Result<Profile, AppError>> {
    try {
      ...
      const uploaded = await uploadFile(bucket, file, userId);
