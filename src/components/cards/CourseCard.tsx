...
export function CourseCard({ course }: Readonly<{ course: Course; dark?: boolean }>) {
  return (
    <article className="premium-surface group flex h-full min-w-0 flex-col overflow-hidden rounded-[20px] border border-[#E5EAF5] bg-white shadow-[0_18px_55px_rgba(10,42,136,0.08)] transition duration-300 hover:-translate-y-1.5 hover:border-[#19D9FF]/45 hover:shadow-[0_24px_75px_rgba(17,71,255,0.16)]">
      <Link to={courseDetailRoute(course.slug)} className="block">
        <div className="relative aspect-[1.48] overflow-hidden bg-[#061B5C]">
          ...
          <div className="absolute left-3 top-3 flex max-w-[calc(100%-4.25rem)] flex-wrap gap-1.5">
            <span className="rounded-md bg-[#19D9FF] px-2.5 py-1 text-[10px] font-black uppercase leading-none text-[#061B5C] shadow-[0_10px_24px_rgba(25,217,255,0.24)]">
              {course.badge}
            </span>
            <span className="rounded-md border border-white/25 bg-[#061B5C]/60 px-2.5 py-1 text-[10px] font-black uppercase leading-none text-white backdrop-blur">
              {course.track}
            </span>
          </div>
          <button type="button" className="absolute right-3 top-3 grid size-9 place-items-center rounded-full border border-white/40 bg-[#061B5C]/40 text-white backdrop-blur transition hover:bg-white hover:text-[#1147FF]" aria-label={`Wishlist ${course.title}`}>
      ...
      <div className="flex flex-1 flex-col p-4">
        <Link to={courseDetailRoute(course.slug)} className="group/title">
          <h3 className="line-clamp-2 text-xl font-black leading-tight text-[#061B5C] transition group-hover/title:text-[#1147FF]">{course.title}</h3>
        </Link>
        ...
        <div className="mt-5 rounded-[16px] bg-[#F8FAFF] p-3">
          <div className="flex items-end justify-between gap-3">
            <div className="min-w-0">
              <p className="text-2xl font-black tracking-tight text-[#061B5C]">{formatPrice(course.price)}</p>
        ...
        <div className="mt-auto pt-5">
          <div className="flex items-center gap-2 border-t border-[#E5EAF5] pt-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <h4 className="line-clamp-2 break-words text-sm font-black leading-5 text-[#061B5C]">{course.mentor}</h4>
            ...
            <Link to={courseDetailRoute(course.slug)} className="grid size-10 shrink-0 place-items-center rounded-full border border-[#E5EAF5] bg-white text-[#061B5C] shadow-[0_10px_28px_rgba(10,42,136,0.08)] transition duration-300 group-hover:translate-x-1 group-hover:border-[#19D9FF] group-hover:bg-[#19D9FF] group-hover:text-[#061B5C]" aria-label={`View ${course.title}`}>
