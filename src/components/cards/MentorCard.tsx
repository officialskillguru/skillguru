  ...
  return (
    <article className="premium-surface group overflow-hidden rounded-xl border border-border bg-card shadow-sm transition duration-300 hover:-translate-y-1.5 hover:border-accent/45 hover:shadow-md">
      <div className="relative h-36 bg-gradient-to-br from-primary to-primary">
        {mentor.avatar ? (
          <img src={mentor.avatar} alt={mentor.name} className="absolute bottom-0 left-5 h-32 w-28 object-cover object-top transition duration-500 group-hover:scale-105" loading="lazy" />
        ) : (
          <div className="absolute bottom-0 left-5 grid h-32 w-28 place-items-center rounded-t-lg bg-primary-foreground/15 text-2xl font-black text-primary-foreground">
      ...
      <div className="p-5">
        ...
        <div className="mt-5 grid grid-cols-3 gap-3 text-xs">
          ...
          <span className="font-bold text-muted-foreground">
            <strong className="flex items-center gap-1 text-primary"><Users className="size-3.5" aria-hidden="true" /> {mentor.studentsMentored}+</strong>
            Students
          </span>
          <span className="font-bold text-muted-foreground">
            <strong className="flex items-center gap-1 text-primary"><Star className="size-3.5 fill-amber-400 text-amber-400" aria-hidden="true" /> {mentor.rating}</strong>
            Rating
          </span>
        </div>
