      ...
      <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        <div className="grid gap-10 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-[1.25fr_repeat(5,0.75fr)_1.15fr]">
          <div>
            ...
            <div className="mt-6 flex gap-3">
              {socialLinks.map(({ icon: Icon, href, label }) => (
                <a key={label} href={href} target="_blank" rel="noopener noreferrer" aria-label={label} className="grid size-9 place-items-center rounded-full bg-white/10 text-primary-foreground/90 transition hover:bg-accent hover:text-accent-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent">
            ...
            <div key={column.title}>
              ...
              <ul className="mt-5 space-y-3">
                {column.links.map(([label, href]) => (
                  <li key={label}>
                    <Link to={href} className="text-sm font-medium text-primary-foreground/70 transition hover:text-accent focus-visible:outline-none focus-visible:text-accent">
                      {label}
                    </Link>
                  </li>
                ))}
          ...
          <div>
            ...
            <div className="mt-5 space-y-4 text-sm text-primary-foreground/80">
              <a href={`tel:${siteConfig.contactPhone.replaceAll(" ", "")}`} className="flex gap-3 hover:text-accent focus-visible:outline-none focus-visible:text-accent">
                <Phone className="mt-0.5 size-4 shrink-0 text-accent" />
                {siteConfig.contactPhone}
              </a>
              <a href={`mailto:${siteConfig.contactEmail}`} className="flex gap-3 hover:text-accent focus-visible:outline-none focus-visible:text-accent">
        ...
        <div className="mt-10 border-t border-white/10 pt-7 text-center text-sm text-primary-foreground/60">
