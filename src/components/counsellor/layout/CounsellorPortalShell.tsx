...
// Mirrors the visual/UX pattern established by MentorPortalShell (same layout
// primitives, same a11y conventions) rather than a shared component, to avoid
// touching the working Mentor portal mid-rollout. Unifying the two shells is a
// reasonable follow-up, not required for this pass.
const navItems = [
  { label: "Dashboard", to: "/counsellor/dashboard", icon: LayoutDashboard },
  { label: "Students", to: "/counsellor/students", icon: Users, section: "Operations" },
  { label: "Mentors", to: "/counsellor/mentors", icon: UserIcon },
  { label: "Courses", to: "/counsellor/courses", icon: BookOpen },
  { label: "Jobs & Internships", to: "/counsellor/jobs", icon: Briefcase, section: "Career" },
  { label: "Messages", to: "/counsellor/messages", icon: MessageSquare, section: "Communication" },
  { label: "Profile", to: "/counsellor/profile", icon: UserIcon, section: "Account" },
] as const;
...
function Sidebar({ collapsed, onNavigate }: Readonly<{ collapsed?: boolean; onNavigate?: () => void }>) {
  ...
  return (
    <div className="flex h-full flex-col bg-sidebar text-sidebar-foreground">
      ...
      <nav aria-label="Counsellor portal" className="enterprise-scrollbar min-h-0 flex-1 overflow-y-auto px-3 py-6">
        {!collapsed && (
        ...
        <div className={`flex items-center gap-3 rounded-lg bg-white/5 p-3 ${collapsed ? "justify-center" : ""}`}>
          ...
          {!collapsed && (
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-semibold text-white">{name ?? "Counsellor"}</p>
...
export function CounsellorPortalShell() {
  const [drawerOpen, setDrawerOpen] = useState(false);
  const [collapsed, setCollapsed] = useState(() => localStorage.getItem("counsellor-sidebar-collapsed") === "1");
  ...
  const toggleCollapsed = () => {
    ...
    setCollapsed(next);
    localStorage.setItem("counsellor-sidebar-collapsed", next ? "1" : "0");
  };
    ...
    >
      ...
      {drawerOpen ? (
        <div ref={drawerContainerRef} className="fixed inset-0 z-50 lg:hidden">
          <button
            type="button"
            aria-label="Close counsellor navigation"
          ...
          <aside
            ...
            role="dialog"
            aria-modal="true"
            aria-label="Counsellor navigation menu"
      ...
      <section className="min-w-0">
        <header className="sticky top-0 z-30 border-b border-border bg-background/95 backdrop-blur-xl supports-backdrop-filter:bg-background/60">
          <div className="flex min-h-[72px] items-center gap-3 px-4 sm:px-6 lg:px-8">
            <button
              ...
              aria-label="Open counsellor navigation"
            ...
            <div className="hidden lg:block">
              <Breadcrumbs pathname={location.pathname} rootPath="/counsellor" rootLabel="Counsellor" />
            </div>
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button
                    ...
                    aria-label="Counsellor account menu"
                  >
                    <span className="relative flex size-9 shrink-0 items-center justify-center rounded-full bg-primary/10 font-bold text-primary">
                      {initials(auth.authUser?.profile?.fullName, "C")}
                    </span>
                    <span className="hidden md:block">
                      <span className="block text-sm font-semibold leading-tight text-foreground">
                        {auth.authUser?.profile?.fullName ?? "Counsellor"}
                      </span>
                      <span className="block text-[11px] font-medium leading-tight text-muted-foreground">Counsellor</span>
                    </span>
                  </button>
                </DropdownMenuTrigger>
        ...
        <main id="counsellor-main-content" className="px-4 py-8 sm:px-6 lg:px-8">
          <Outlet />
        </main>
      </section>
    </div>
