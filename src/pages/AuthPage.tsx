  ...
  return (
    <div className="min-h-svh bg-white">
      ...
      <main className="grid bg-white lg:grid-cols-[0.95fr_1.05fr]">
        <section className="relative min-h-170 overflow-hidden bg-[#111E79] p-8 text-white sm:p-12">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_68%_28%,rgba(25,217,255,0.25),transparent_28rem)]" />
          <div className="relative flex h-full flex-col">
            <Logo onDark />
            <div className="mt-20 max-w-xl">
              <h1 className="text-4xl font-black leading-tight sm:text-5xl">Welcome Back! <span className="block text-[#19C7C8]">Glad to see you again</span></h1>
                ...
                {[
                  ...
                  return (
                    <div key={item.title} className="flex gap-5">
                      <span className="grid size-14 shrink-0 place-items-center rounded-md bg-[#19C7C8]/12 text-[#19C7C8]">
        ...
        <section className="flex items-center justify-center bg-[#F8FAFF] px-4 py-12 sm:px-6 lg:px-8">
          <form onSubmit={(event) => void onSubmit(event)} className="w-full max-w-150 rounded-3xl border border-[#E5EAF5] bg-white p-7 shadow-[0_28px_90px_rgba(10,42,136,0.12)] sm:p-10">
            <div className="grid grid-cols-2 border-b border-[#E5EAF5] text-center text-xl font-black">
              <Link to={routes.login} className={mode === "login" ? "border-b-4 border-[#5B35F2] px-4 pb-5 text-[#111E79]" : "px-4 pb-5 text-[#64748B]"}>
                Login
              </Link>
              <Link to={routes.signup} className={mode === "signup" ? "border-b-4 border-[#5B35F2] px-4 pb-5 text-[#111E79]" : "px-4 pb-5 text-[#64748B]"}>
                Sign Up
              </Link>
            </div>
            ...
            {mode === "signup" ? (
              <div className="mt-3">
                <div className="h-2 overflow-hidden rounded-full bg-[#E5EAF5]">
                  <div className="h-full rounded-full bg-[#5B35F2]" style={{ width: `${strength}%` }} />
                </div>
                <p className="mt-2 text-xs font-semibold text-[#64748B]">Password strength</p>
              </div>
            ) : (
              <Link to={routes.contact} className="mt-5 block text-right text-sm font-black text-[#5B35F2]">Forgot Password?</Link>
            )}
            <button type="submit" className="mt-7 inline-flex h-14 w-full items-center justify-center gap-3 rounded-md bg-[#5B35F2] text-base font-black text-white shadow-[0_16px_42px_rgba(17,71,255,0.24)] transition hover:-translate-y-1 hover:bg-[#111E79]">
            ...
            <div className="mt-8 grid grid-cols-4 gap-4">
              {[
                ...
                const Icon = item.icon;
                return (
                <button key={index} type="button" className="grid h-16 place-items-center rounded-md border border-[#E5EAF5] bg-white text-2xl font-black text-[#111E79] transition hover:-translate-y-1 hover:border-[#5B35F2]">
            ...
            <div className="mt-8 flex gap-4 rounded-2xl bg-[#F8FAFF] p-5">
              <ShieldCheck className="size-10 shrink-0 rounded-full bg-white p-2 text-[#5B35F2]" />
              <div>
                <p className="font-black text-[#111E79]">Your data is safe with us</p>
...
function Field({
  ...
  return (
    <label className="mt-7 block">
      <span className="text-sm font-black text-[#111E79]">{label}</span>
      <span className="mt-3 flex h-14 items-center gap-3 rounded-md border border-[#E5EAF5] px-4 text-sm text-[#111E79] focus-within:border-[#5B35F2]">
