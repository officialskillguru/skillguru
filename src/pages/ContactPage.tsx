    ...
    <main className="page-shell">
      <section className="relative overflow-hidden bg-[#111E79] px-4 py-12 text-white sm:px-6 lg:px-8 lg:py-16">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-10 lg:grid-cols-[0.85fr_1.15fr] lg:items-start">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.22em] text-[#19C7C8]">Contact Us</p>
              ...
              {[
                ...
                return (
                  <article key={item.title} className="rounded-lg border border-white/15 bg-white/8 p-5">
                    <Icon className="size-7 text-[#19C7C8]" />
            ...
            <div className="mt-8 flex max-w-xl items-center justify-between rounded-lg bg-white p-5 text-[#111E79] shadow-[0_20px_70px_rgba(2,8,23,0.16)]">
          ...
          <form onSubmit={(event) => void onSubmit(event)} className="rounded-lg border border-[#E5EAF5] bg-white p-6 text-[#111E79] shadow-[0_24px_80px_rgba(2,8,23,0.18)] sm:p-8">
            <h2 className="text-2xl font-black">Send Message</h2>
            <div className="mt-6 grid gap-4 sm:grid-cols-2">
              <input name="fullName" required placeholder="Full Name" className="h-12 rounded-md border border-[#E5EAF5] px-4 text-sm outline-none focus:border-[#5B35F2]" />
              <input name="phone" required placeholder="Phone Number" className="h-12 rounded-md border border-[#E5EAF5] px-4 text-sm outline-none focus:border-[#5B35F2]" />
              <input name="email" required type="email" placeholder="Email Address" className="h-12 rounded-md border border-[#E5EAF5] px-4 text-sm outline-none focus:border-[#5B35F2]" />
              <select name="subject" required className="h-12 rounded-md border border-[#E5EAF5] px-4 text-sm outline-none focus:border-[#5B35F2]">
            ...
            <textarea name="message" required rows={5} placeholder="How can we help you?" className="mt-4 w-full rounded-md border border-[#E5EAF5] px-4 py-3 text-sm outline-none focus:border-[#5B35F2]" />
            <label className="mt-4 flex gap-3 text-sm text-[#64748B]">
              <input name="consent" type="checkbox" required className="mt-1 accent-[#5B35F2]" />
              I agree to be contacted by SkillGuru.
            </label>
            <button type="submit" className="mt-6 inline-flex h-12 w-full items-center justify-center gap-2 rounded-md bg-[#5B35F2] text-sm font-black text-white transition hover:-translate-y-1 hover:bg-[#111E79]">
            ...
            {status ? <p className="mt-4 rounded-md bg-[#F1F5FF] p-3 text-sm font-semibold text-[#5B35F2]">{status}</p> : null}
          </form>
        </div>
      </section>
      ...
      <section className="bg-white px-4 py-12 sm:px-6 lg:px-8 lg:py-25">
        <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.9fr_1.1fr]">
          <div>
            <h2 className="text-2xl font-black text-[#111E79]">Our Offices</h2>
            ...
            <div className="mt-8 grid gap-5 md:grid-cols-3 lg:grid-cols-1 xl:grid-cols-3">
              {officeLocations.map((office) => (
                <article key={office.city} className="rounded-lg border border-[#E5EAF5] bg-white p-5 shadow-[0_16px_50px_rgba(10,42,136,0.08)]">
                  <MapPin className="size-10 rounded-full bg-[#E8F9FF] p-2 text-[#5B35F2]" />
                  <h3 className="mt-5 font-black text-[#111E79]">{office.city} <span className="text-xs text-[#64748B]">({office.label})</span></h3>
                  <p className="mt-3 text-sm leading-6 text-[#64748B]">{office.address}</p>
                  <p className="mt-4 text-xs font-black text-[#111E79]">{office.phone}</p>
                  <p className="mt-2 text-xs font-black text-[#111E79]">{office.email}</p>
                </article>
              ))}
            </div>
          </div>
          <div className="relative min-h-90 overflow-hidden rounded-lg border border-[#E5EAF5] bg-[linear-gradient(135deg,#DFF4FF,#F8FAFF)] shadow-[0_18px_60px_rgba(10,42,136,0.08)]">
            ...
            {officeLocations.map((office, index) => (
              <div key={office.city} className="absolute rounded-[12px] bg-white px-4 py-2 text-sm font-black text-[#111E79] shadow-[0_12px_30px_rgba(10,42,136,0.16)]" style={{ left: `${20 + index * 24}%`, top: `${22 + index * 18}%` }}>
                <MapPin className="mr-2 inline size-4 text-[#5B35F2]" />
                {office.city}
              </div>
            ))}
            <Navigation className="absolute bottom-8 right-8 size-12 text-[#5B35F2]" />
          </div>
        </div>
      </section>
          ...
          {supportOptions.map((item) => {
            ...
            return (
              <article key={item.title} className="rounded-lg border border-[#E5EAF5] bg-white p-6 shadow-[0_16px_45px_rgba(10,42,136,0.07)]">
                <Icon className="size-12 rounded-full bg-[#F1F5FF] p-3 text-[#5B35F2]" />
                ...
                <a
                  ...
                  className="mt-4 inline-flex items-center gap-2 text-sm font-black text-[#5B35F2]"
                >
                  {item.action} -&gt;
                </a>
              </article>
      ...
      <section className="bg-white px-4 py-12 sm:px-6 lg:px-8 lg:py-25">
        <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.7fr_1.3fr]">
          <div className="rounded-lg border border-[#E5EAF5] bg-[#F8FAFF] p-7">
            <h2 className="text-3xl font-black text-[#111E79]">Frequently Asked Questions</h2>
